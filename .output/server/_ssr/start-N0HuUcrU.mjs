...
//#region node_modules/.nitro/vite/services/ssr/assets/start-N0HuUcrU.js
...
function createSupabaseClient() {
	const config = getSupabaseConfig({ runtime: "client" });
	if (!hasSupabaseConfig(config)) {
		const message = `Missing Supabase environment variable(s): ${[...!config.url ? ["SUPABASE_URL"] : [], ...!config.key ? ["SUPABASE_PUBLISHABLE_KEY"] : []].join(", ")}. Connect Supabase in Lovable Cloud.`;
